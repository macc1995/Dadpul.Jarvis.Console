...
internal sealed class JarvisConsoleApplication
{
    ...
    ChatMetrics? metrics)
    {
        ...
        System.Console.WriteLine(
            $"Finish reason:      {metrics.FinishReason ?? "unknown"}");

        System.Console.WriteLine();

        System.Console.WriteLine(
            $"Prompt tokens:      {metrics.PromptTokenCount:N0}");

        System.Console.WriteLine(
            $"Generated tokens:   {metrics.GeneratedTokenCount:N0}");

        System.Console.WriteLine();

        System.Console.WriteLine(
            $"Model load:         {FormatDuration(metrics.LoadDuration)}");

        System.Console.WriteLine(
            $"Prompt evaluation:  {FormatDuration(metrics.PromptEvaluationDuration)}");

        System.Console.WriteLine(
            $"Token generation:   {FormatDuration(metrics.GenerationDuration)}");

        System.Console.WriteLine(
            $"Total Ollama time:  {FormatDuration(metrics.TotalDuration)}");

        System.Console.WriteLine();

        System.Console.WriteLine(
            $"Prompt speed:       {metrics.PromptTokensPerSecond:N2} tokens/s");

        System.Console.WriteLine(
            $"Generation speed:   {metrics.GenerationTokensPerSecond:N2} tokens/s");
    ...
    private void PrintHistory()
    {
        ...
        foreach (ChatMessage message in conversation.Messages.Where(message => message.Role != ChatRole.System))
        {
            System.Console.WriteLine(
