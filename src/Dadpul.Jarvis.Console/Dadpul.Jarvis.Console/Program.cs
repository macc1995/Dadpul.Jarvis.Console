...
namespace Dadpul.Jarvis.Console
{
   ...
   internal static class Program
   {
      ...
      private static AggregateCatalog CreateMefCatalog()
      {
         ...
         return new AggregateCatalog(new AssemblyCatalog(Assembly.GetExecutingAssembly()),
      ...
      {
         var compositionContainer = new CompositionContainer(catalog, isThreadSafe: true);
      ...
      private static IConfigurationRoot LoadConfig()
      {
         return new ConfigurationBuilder().SetBasePath(AppContext.BaseDirectory)
            .AddJsonFile("appsettings.json", optional: false, reloadOnChange: false).AddUserSecrets(Assembly.GetExecutingAssembly(), optional: true)
      ...
      private static async Task Main(string[] args)
      {
         ...
         using var compositionContainer = CreateMefContainer(catalog);
         ...
         {
            BaseAddress = ollamaOptions.BaseAddress ?? throw new InvalidOperationException("Ollama:BaseAddress was not configured.")
         };
         ...
         var largeOptions = configuration.GetSection("ChatModels:EmbeddedLarge").Get<LLamaSharpModelOptions>()
                            ?? throw new InvalidOperationException("Embedded large model configuration is missing.");

         var smallOptions = configuration.GetSection("ChatModels:EmbeddedSmall").Get<LLamaSharpModelOptions>()
                            ?? throw new InvalidOperationException("Embedded small model configuration is missing.");

         models.Add(new LLamaSharpChatModel(largeOptions, new EmbeddedFallbackSystemPrompt()));

         models.Add(new LLamaSharpChatModel(smallOptions, new EmbeddedEmergencyFallbackSystemPrompt()));
         ...
         var frontends = new List<IFrontend> { new ConsoleFrontend() };
      ...
      private static async Task PreloadModelAsync(HttpClient httpClient, string model, int keepAlive, CancellationToken cancellationToken = default)
      {
         ...
         using var response = await httpClient.PostAsJsonAsync("api/generate", new { model, keep_alive = keepAlive }, cancellationToken);
      ...
      private static ServiceCollection RegisterServices(IConfigurationRoot configuration)
      {
         ...
         services.AddOptions<OllamaOptions>().Bind(configuration.GetRequiredSection(OllamaOptions.SectionName))
            .Validate(options => options.BaseAddress is not null, "Ollama:BaseAddress is required.")
         ...
         services.AddOptions<DiscordOptions>().Bind(configuration.GetRequiredSection(DiscordOptions.SectionName)).Validate(
            options => !options.Enabled || !string.IsNullOrWhiteSpace(options.Token), "Discord:Token is required when Discord is enabled.");
      ...
      private static async Task<IChatModel> StartOllama(OllamaOptions ollamaOptions, HttpClient httpClient, CompositionBatch compositionBatch,
         CancellationTokenSource cancellationTokenSource)
      {
         ...
         {
            await PreloadModelAsync(httpClient, ollamaOptions.Model, ollamaOptions.Preload.KeepAlive, cancellationTokenSource.Token);
         }
