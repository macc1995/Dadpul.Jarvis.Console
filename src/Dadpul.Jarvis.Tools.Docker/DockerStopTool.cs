...
namespace Dadpul.Jarvis.Tools.Docker
{
   ...
   [Export(typeof(ITool))]
   public class DockerStopTool : DockerHandling, ITool
   {
      #region Constructors and Destructors

      [ImportingConstructor]
      public DockerStopTool(IDockerController dockerController)
      ...
      public string Description =>
         "Stops one exact Docker container. Use docker_list_containers first "
         + "unless the exact host and container name are already available from "
         + "recent tool output. When exactly one listed container clearly matches "
         + "the user's request, call this tool in the same turn. Only stop a " + "container when the user explicitly requests it.";

      public string Name { get; } = "docker_stop";
      ...
      public JsonObject Parameters =>
         new()
         {
            ...
            ["properties"] = new JsonObject
            {
               ...
               {
                  ["type"] = "string", ["description"] = "The exact container name returned by docker_list_containers."
               }
            },
      ...
      public async Task<ToolResult> ExecuteAsync(JsonObject arguments, CancellationToken cancellationToken)
      {
         ...
         try
         {
            var result = await dockerController.StopContainerAsync(node, containerName, cancellationToken);
