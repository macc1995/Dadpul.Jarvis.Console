...
internal sealed class GitHubApiClient : IGitHubApiClient, IDisposable
{
   ...
   public GitHubApiClient(IConfiguration configuration, IGitHubAuthenticationProvider authenticationProvider)
   {
      ArgumentNullException.ThrowIfNull(configuration);
      ArgumentNullException.ThrowIfNull(authenticationProvider);

      var options = configuration.GetRequiredSection(GitHubOptions.SectionName).Get<GitHubOptions>()
      ...
      allowedRepositories = options.AllowedRepositories.Select(NormalizeRepository).ToHashSet(StringComparer.OrdinalIgnoreCase);

      if (!allowedRepositories.Contains(DefaultRepository))
      {
         throw new GitHubClientException($"GitHub:DefaultRepository '{DefaultRepository}' must also be present in GitHub:AllowedRepositories.");
      }

      httpClient = new HttpClient
      {
         BaseAddress = EnsureTrailingSlash(options.ApiBaseAddress!), Timeout = TimeSpan.FromSeconds(options.RequestTimeoutSeconds)
      };
   ...
   public async Task<T> GetAsync<T>(string? repository, string relativePath, CancellationToken cancellationToken)
   {
      ...
      try
      {
         return await response.Content.ReadFromJsonAsync<T>(JsonOptions, cancellationToken)
                ?? throw new GitHubClientException($"GitHub returned an empty response for repository '{selectedRepository}'.");
      }
      catch (JsonException exception)
      {
         throw new GitHubClientException($"GitHub returned invalid JSON for repository '{selectedRepository}'.", exception);
      }
   }

   public string ResolveRepository(string? repository)
   {
      var selectedRepository = string.IsNullOrWhiteSpace(repository) ? DefaultRepository : NormalizeRepository(repository);

      if (!allowedRepositories.Contains(selectedRepository))
      {
         throw new GitHubClientException($"Repository '{selectedRepository}' is not allowed. Configured repositories: "
   ...
   private static async Task EnsureSuccessfulAsync(string repository, HttpResponseMessage response, CancellationToken cancellationToken)
   {
      ...
      var safeBody = responseBody.Length <= 1_000 ? responseBody : responseBody[..1_000] + "…";

      throw new GitHubClientException($"GitHub returned HTTP {(int)response.StatusCode} ({response.ReasonPhrase}) "
   ...
   private static string NormalizeRepository(string repository)
   {
      ...
      if ((segments.Length != 2) || segments.Any(segment => (segment.Length == 0) || segment.Any(char.IsWhiteSpace)))
      {
         throw new GitHubClientException($"Repository '{repository}' must use the 'owner/name' format.");
      }
   ...
   private static void ValidateOptions(GitHubOptions options)
   {
      ...
      if ((options.ApiBaseAddress.Scheme != Uri.UriSchemeHttps) && (options.ApiBaseAddress.Scheme != Uri.UriSchemeHttp))
      {
